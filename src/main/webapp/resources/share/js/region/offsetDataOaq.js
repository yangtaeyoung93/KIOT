        ...
        columnDefs: [ 
        	...
        	},
        	{
	        	...
	        	render: function(data, type, full, meta) {
	        		...
	        		else linkHtml += "<a href='#' onClick='viewData2(\"" + full.serial + "\",\"" + parentSpaceName
	        			+ "\",\"" + spaceName+"\",\"" + full.productDt+"\",\"" + full.stationName + "\");', 'sum'>" + full.serial + "</a>";
...
function getHighSpace(){
	...
	$.ajax({
		...
		success : function(param) {
			...
			for (var i = 0; i < param.data.length; i++) optHtml += "<option value='"
