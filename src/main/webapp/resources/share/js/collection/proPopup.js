			...
			$.ajax({
				...
				success : function(d) {
					...
					if (elemetsData.length != 0) {
						...
						for (var index in elemetsData) {
							columsData.push({
								"data" : elemetsData[index].engName
							});
							collectionColum.push(elemetsData[index].engName);
						}

						// ===========================================
						// ================== 예외 처리 ==================
						if (!collectionColum.includes("noise")) {
							columsData.push({
								"data" : "noise"
							});
							collectionColum.push("noise");
							elemetsData.push({
								"rowNum" : 0,
								"idx" : 0,
								"korName" : "소음",
